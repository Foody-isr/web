...
// iOS gives date/time controls an intrinsic width wider than their parent.
// Explicitly allow every field to shrink inside the mobile card and neutralize
// the native appearance that otherwise paints outside the input's box.
...
const DATE_TIME_FIELD = `${FIELD} min-h-12 [&::-webkit-date-and-time-value]:min-w-0 [&::-webkit-date-and-time-value]:text-start`;
    ...
    return (
      <div className="min-w-0 overflow-hidden rounded-2xl border border-[var(--divider)] bg-[var(--surface-subtle)] p-4">
        <label className="block min-w-0 max-w-md">
          ...
          <input className={DATE_TIME_FIELD} type="date" value={session?.date ?? ""} onChange={(event) => updateDate(event.target.value)} />
        </label>
      </div>
    );
  }
