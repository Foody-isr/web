...
/**
 * Social media links section with platform icons.
 * Content: links [{platform, url}]
 */
export function SocialFeedSection({ section }: SectionProps) {
  const links: SocialLink[] = section.content?.links || [];

  if (links.length === 0) return null;

  return (
    <section className="bg-[var(--surface)] py-12 px-6">
      <div className="max-w-4xl mx-auto flex flex-wrap justify-center gap-6">
        {links.map((link, i) => {
          const icon = PLATFORM_ICONS[link.platform.toLowerCase()];
          const label = icon?.label || link.platform;
